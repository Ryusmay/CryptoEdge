...
function Test-CryptoEdgeApi {
    try {
        ...
        if ($health.app -eq "CryptoEdge") { return $true }
        # Zgodność z backendami CryptoEdge sprzed pola `app`. Samo `ok=true`
        # nie wystarcza, bo na porcie mógłby działać obcy lokalny serwis.
        return (($health.version -as [string]) -match '^\d+\.\d+\.\d+$' -and
                ($health.url -as [string]) -match '^http://127\.0\.0\.1:47821/?$')
    } catch {
        return $false
    }
}
