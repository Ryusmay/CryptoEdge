...
def _liveness(rt) -> dict:
    """Wiek ostatniego cyklu i mapy cen. Patrz BotRuntime.liveness()."""
    try:
        return rt.liveness()
    except Exception as exc:
        return {"state": "unknown", "error": str(exc)[:160]}
...
class EngineApi:
    ...
    def handle_get(self, handler):
        ...
        if path in ("/health", "/api/health"):
            ...
            self._send(handler, 200, {"ok": True, "version": version.display(),
